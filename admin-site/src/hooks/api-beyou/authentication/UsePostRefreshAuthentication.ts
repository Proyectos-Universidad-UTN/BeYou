...
export const UsePostRefreshAuthentication = ({
  ...
  const postRefreshToken = UseTypedApiClientBY({ path, method });

  return useMutation({
    mutationFn: async (tokenRefreshModel: UserTokenRefreshRequest) => {
      const { data } = await postRefreshToken(
        castRequestBody(tokenRefreshModel, path, method)
      );
      return data;
    },
    onSuccess,
