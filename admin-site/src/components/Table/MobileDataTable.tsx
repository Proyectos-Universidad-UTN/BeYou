...
const handleCheckBoxOnClick = <RowModelT extends GridValidRowModel>(
    ...
    if (model.includes(row.id)) {
        onRowSelectionModelChange(
            compact(reject(model, (id) => id === row.id)) as unknown as GridRowSelectionModel,
            { api }
        )
        return
    }

    onRowSelectionModelChange([...model, row.id] as unknown as GridRowSelectionModel, { api })
}
...
const MobileRow = <RowModelT extends GridValidRowModel>({
    ...
    'columns' | 'checkboxSelection' | 'rowSelectionModel' | 'onRowSelectionModelChange'
> & {
    row: RowModelT
}) => {
            ...
            <div>
                {columns.map((col, index) => {
                    ...
                    const valueRow = ((): string => {
                        ...
                        return String(row[field as keyof RowModelT])
                    })()
                    ...
                    <Box
                        ...
                        onClick={(
                            ...
                            if (checkboxSelection) {
                                ...
                                onRowClick(
                                    { row, id: row.id, columns: columns as GridColDef<RowModelT>[] },
                                    event,
                                    { api }
                                )
                            }
